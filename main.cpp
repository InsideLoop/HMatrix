...
int main() {
  ...
  while (true) {
    ...
    il::Array2DEdit<double> ref_frobenius_A = frobenius_A.edit();
    il::blas(1.0,
             A.view(il::Range{0, n0 * p}, il::Range{rank * p, (rank + 1) * p}),
             transpose,
             A.view(il::Range{0, n0 * p}, il::Range{rank * p, (rank + 1) * p}),
             0.0, il::io, ref_frobenius_A);
    // New value for the norm of B
    il::StaticArray2D<double, p, p> frobenius_B{0.0};
    il::Array2DEdit<double> ref_frobenius_B = frobenius_B.edit();
    il::blas(1.0,
             B.view(il::Range{rank * p, (rank + 1) * p}, il::Range{0, n1 * p}),
             B.view(il::Range{rank * p, (rank + 1) * p}, il::Range{0, n1 * p}),
             transpose, 0.0, il::io, ref_frobenius_B);
    // compute ||A_k B_k||^2
    ...
    //    il::Array2D<double> low_rank = hmat::lowRankApproximation(M, A, B,
    //    rank); const double forbenius_norm_low_rank =
    //    hmat::frobeniusNorm(low_rank);
